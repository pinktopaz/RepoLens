...
<!-- 이유: v4는 v3과 거의 다른 프레임워크 수준의 변화가 있음. AI는 v3 기준으로 훈련되었는데, v3 방식으로 코드를 쓰면 스타일이 아예 안 먹힘.
- v3은 빌드 시점에 클래스 목록을 스캔해서 CSS를 생성하는 방식 (tailwind.config.js에 theme 설정 → Tailwind가 읽음)
- v4는 그 구조를 버리고 CSS 엔진을 새로 만들어서 진입점이 바뀜 (tailwind.config.js 무시됨 → globals.css의 @theme 블록만 읽음)
-->
...
- 클래스명 단축 변경: `flex-shrink-0` → `shrink-0`, `flex-grow` → `grow` 등
